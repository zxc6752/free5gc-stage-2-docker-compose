...
def get_ave_info_from_iperf(types):
    ...
    ave_loss_rate_sum = np.array([0.0] * 80)
    data_cnt = np.array([1] * 80)
    throughput_sum = np.array([0.0] * 80)
    for i in range(0,16):
        index = 2*i
        length = len(file_loss_list[i])-1
        ave_loss_rate_sum[index:index+length] += np.array(file_loss_list[i][0:length])
        data_cnt[index:index+length] += np.array([1] * length)
        throughput_sum[index:index+length] += np.array(file_throughput_list[i][0:length])
...
plt.xlabel('Time(Sec)')
plt.ylabel('%')
plt.title('Packet Loss Rate',fontsize=16, fontweight='bold')
plt.legend()
...
plt.xlabel('Time(Sec)')
plt.ylabel('Mbps')
plt.title('Throughput',fontsize=16, fontweight='bold')
plt.legend()
...
plt.yticks(np.arange(0.0, 1000.0, 50.0))
plt.xticks(x1)
plt.xlabel('PDU Session ID')
plt.ylabel('ms')
plt.title('Round-Trip Time',fontsize=16, fontweight='bold')
